...
export default {
  ...
  data () {
    return {
      columns: [
        ...
        { title: '显示名称', key: 'displayName', width: 200 },
        { title: '数据字典编号', key: 'dataDictId', width: 200 }
      ],
      dataSet: [],
      selectDataRow: [],
  ...
  methods: {
    ...
    getDataSet () {
      this.spinShow = true
      this.pageShow = true
      this.pagingGetDataDictValue({ rows: this.rows, page: this.page }).then(res => {
    ...
    keywordSearch (keyLikeValue) {
      if (keyLikeValue !== '') {
        ...
        this.queryDataDictValue(dataDictValueQueryKeyLikeValue).then(res => {
          if (res.data.isSuccess && res.data.statusCode === 200) {
            this.dataSet = res.data.data
            this.count = res.data.count
            this.setSelectDataRow([])
    ...
    vagueQuery () {
      ...
      this.pageShow = false
      this.spinShow = true
      this.queryDataDictValue(this.dataDictValueVagueQuery).then(res => {
        if (res.data.isSuccess && res.data.statusCode === 200) {
          this.dataSet = res.data.data
          this.count = res.data.count
          this.setSelectDataRow([])
